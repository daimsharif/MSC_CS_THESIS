...
from torchio import ToCanonical, Resample
...
INFO_FILE = data_path.parent / '211230_M&Ms_Dataset_information_diagnosis_opendataset.csv' # outside train folder
# INFO_FILE = data_path.parent / '210219_M&Ms Dataset information - diagnosis - opendataset.csv'
RESULTS_FOLDER = HOME_PATH / Path(config['paths']['misc'])

ACDC_TRAIN_PATH = HOME_PATH / Path(config['paths']['ACDC']['processed']).joinpath('Training')

# print(f"HOME_PATH: {HOME_PATH.resolve()}")
# print(f"config_file: {config_file.resolve()}")
# print(f"data_path: {data_path.resolve()}")
# print(f"TRAIN_FOLDER: {TRAIN_FOLDER.resolve()}")
# print(f"VAL_FOLDER: {VAL_FOLDER.resolve()}")
# print(f"TEST_FOLDER: {TEST_FOLDER.resolve()}")
# print(f"INFO_FILE: {INFO_FILE.resolve()}")
# print(f"RESULTS_FOLDER: {RESULTS_FOLDER.resolve()}")
# print(f"ACDC_TRAIN_PATH: {ACDC_TRAIN_PATH.resolve()}")
...
class MultiDataLoader():
    def __init__(self, data_owners=None, images_dir=TRAIN_FOLDER, split_ratio=0.1, no_split=False):
        ...
        for subject_name in subject_names:
            ...
            if config['data']['concatenateEDES']:
                ...
                # print(f"Subjects before: {self.subjects}")
        ...
        if config['federated']['isTrue']:
            ...
            # print(f"Subjects after: {self.subjects}")
            ...
            if config['cvtype'] == 'LCO':
                ...
                print(f"uc: {unique_classes}, c: {counts}, mcc: {min_class_count}, len y: {len(y)}")
                # Choose safe n_splits
                n_splits = min(5, min_class_count)

                if n_splits < 2:
                    raise ValueError(f"Too few samples in one or more classes (min={min_class_count}) for splitting.")
    ...
    # Transform and define dataset object
        ...
        if transformations == 'B':
            training_transform = Compose([
                ToCanonical(),  # unify orientation
                Resample(target='mri'),  # align all images to mri spacing
    ...
    def load(self, fold_index, transformations=config['data']['transformations'], LCO_FL_test=False):
        ...
        training_loader = torch.utils.data.DataLoader(
            training_set, batch_size=training_batch_size,num_workers=4,pin_memory=False,persistent_workers=True,prefetch_factor=2)
        validation_loader = torch.utils.data.DataLoader(
            validation_set, batch_size=test_batch_size, num_workers=2,pin_memory=False,persistent_workers=True,prefetch_factor=2)
        if not test_set:
            return training_loader, validation_loader, None 
        test_loader = torch.utils.data.DataLoader(
            test_set, batch_size=test_batch_size,num_workers=2,pin_memory=False,persistent_workers=True,prefetch_factor=2)
